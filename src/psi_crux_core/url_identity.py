...
normalized, KNOWN tracking params stripped, content params PRESERVED — the DB key) · final_url
(where PSI's redirects landed) · storage_key_url (= canonical_url). Never fetches to resolve.
"""
...
def canonicalize(url: str) -> str:
    """Deterministic canonical form. Content params kept; tracking params dropped."""
    u = url.strip()
    if "://" not in u:
        u = "https://" + u
    parts = urlsplit(u)
